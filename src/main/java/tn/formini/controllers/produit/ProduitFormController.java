...
public class ProduitFormController implements Initializable {
    ...
    private boolean validatePrix() {
        ...
        try {
            ...
            if (prix.compareTo(new BigDecimal("999999.999")) > 0) {
                errPrix.setText("Le prix ne peut pas dépasser 999999.999 DT.");
                errPrix.setVisible(true);
                return false;
            }
        } catch (NumberFormatException e) {
            errPrix.setText("Le prix doit être un nombre valide (ex: 19.999 DT).");
            errPrix.setVisible(true);
            return false;
        }
