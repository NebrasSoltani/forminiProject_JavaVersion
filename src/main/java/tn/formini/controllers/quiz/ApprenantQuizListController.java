...
public class ApprenantQuizListController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        // Auto-load all quizzes on startup
        chargerTousLesQuiz();
    }

    /** Charge tous les quiz disponibles (sans filtre par formation) */
    private void chargerTousLesQuiz() {
        quizContainer.getChildren().clear();
        statusLabel.setText("");
        progressLabel.setText("5 / 5");

        tn.formini.services.quizService.QuizService quizService = new tn.formini.services.quizService.QuizService();
        List<Quiz> quizzes = quizService.getAll();

        if (quizzes == null || quizzes.isEmpty()) {
            statusLabel.setText("Aucun quiz disponible dans la base de données.");
            return;
        }

        for (Quiz quiz : quizzes) {
            VBox card = createQuizCard(quiz, 0, true);
    ...
    public void chargerQuiz() {
        ...
        // If no formation ID given, load all quizzes
        ...
        try {
            ...
            if (quizzes == null || quizzes.isEmpty()) {
                // Fallback: load all quizzes if none found for this formation
                statusLabel.setText("Aucun quiz pour la formation " + formationId + " — affichage de tous les quiz.");
                chargerTousLesQuiz();
                return;
            }
